...
export default function ChipInput({
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      <label className="text-sm dark:text-dark-richblack-5 text-light-richblack-5" htmlFor={name}>
      ...
      <div className="flex w-full flex-wrap gap-y-2">
        ...
        {chips.map((chip, index) => (
          <div
            key={index}
            className="m-1 flex items-center rounded-full dark:bg-dark-yellow-200 bg-light-yellow-900 px-2 py-1 text-sm dark:text-dark-richblack-5 text-light-richblack-5"
          >
      ...
      {errors[name] && (
        <span className="ml-2 text-xs tracking-wide dark:text-dark-pink-200 text-light-pink-200">
          {label} is required
        </span>
      )}
    </div>
